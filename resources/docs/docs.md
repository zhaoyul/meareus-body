...
The `home-routes` handler in the `size-measure.routes.home` namespace
...
The `home-page` function will in turn call the `size-measure.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/size_measure/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `size-measure.handler` namespace:
...
Request middleware functions are located under the `size-measure.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `size-measure.dev-middleware` namespace found in
